...
--==================================================
-- BRING UNANCHORED PARTS GUI
--==================================================

local BringGui = Instance.new("Frame")
BringGui.Name = "BringPartsFrame"
BringGui.Size = UDim2.new(0, 320, 0, 140)
BringGui.Position = UDim2.new(0.5, -160, 0.5, -70)
BringGui.BackgroundColor3 = Color3.fromRGB(10, 10, 16)
BringGui.BorderSizePixel = 0
BringGui.ClipsDescendants = true
BringGui.Visible = false
BringGui.Parent = Gui

local BringCorner = Instance.new("UICorner")
BringCorner.CornerRadius = UDim.new(0, 8)
BringCorner.Parent = BringGui

local BringStroke = Instance.new("UIStroke")
BringStroke.Color = MAIN_COLOR
BringStroke.Thickness = 2
BringStroke.Parent = BringGui

local BringTopBar = Instance.new("Frame")
BringTopBar.Size = UDim2.new(1, 0, 0, 32)
BringTopBar.BackgroundColor3 = MAIN_COLOR
BringTopBar.BorderSizePixel = 0
BringTopBar.Parent = BringGui

local BringTitle = Instance.new("TextLabel")
BringTitle.Size = UDim2.new(1, -40, 1, 0)
BringTitle.Position = UDim2.new(0, 10, 0, 0)
BringTitle.BackgroundTransparency = 1
BringTitle.Text = "Bring Parts GUI"
BringTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
BringTitle.Font = Enum.Font.GothamBold
BringTitle.TextSize = 13
BringTitle.TextXAlignment = Enum.TextXAlignment.Left
BringTitle.Parent = BringTopBar

local BringCloseBtn = Instance.new("TextButton")
BringCloseBtn.Size = UDim2.new(0, 24, 0, 22)
BringCloseBtn.Position = UDim2.new(1, -28, 0.5, -11)
BringCloseBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
BringCloseBtn.BorderSizePixel = 0
BringCloseBtn.Text = "X"
BringCloseBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
BringCloseBtn.Font = Enum.Font.GothamBold
BringCloseBtn.TextSize = 11
BringCloseBtn.Parent = BringTopBar

local BringCloseCorner = Instance.new("UICorner")
BringCloseCorner.CornerRadius = UDim.new(0, 4)
BringCloseCorner.Parent = BringCloseBtn

BringCloseBtn.MouseButton1Click:Connect(function()
    BringGui.Visible = false
end)

local BDragging, BDragInput, BDragStart, BStartPos
BringTopBar.InputBegan:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
		BDragging = true
		BDragStart = input.Position
		BStartPos = BringGui.Position
		input.Changed:Connect(function()
			if input.UserInputState == Enum.UserInputState.End then BDragging = false end
		end)
	end
end)
BringTopBar.InputChanged:Connect(function(input)
	if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
		BDragInput = input
	end
end)
UserInputService.InputChanged:Connect(function(input)
	if input == BDragInput and BDragging then
		...
		BringGui.Position = UDim2.new(BStartPos.X.Scale, BStartPos.X.Offset + Delta.X, BStartPos.Y.Scale, BStartPos.Y.Offset + Delta.Y)
	end
end)

local BringContent = Instance.new("Frame")
BringContent.Size = UDim2.new(1, -16, 1, -40)
BringContent.Position = UDim2.new(0, 8, 0, 36)
BringContent.BackgroundTransparency = 1
BringContent.Parent = BringGui

local BringNickInput = Instance.new("TextBox")
BringNickInput.Size = UDim2.new(1, 0, 0, 30)
BringNickInput.Position = UDim2.new(0, 0, 0, 8)
BringNickInput.BackgroundColor3 = Color3.fromRGB(22, 22, 32)
BringNickInput.BorderSizePixel = 0
BringNickInput.PlaceholderText = "Target Nick / DisplayName..."
BringNickInput.Text = ""
BringNickInput.TextColor3 = Color3.fromRGB(255, 255, 255)
BringNickInput.PlaceholderColor3 = Color3.fromRGB(150, 150, 150)
BringNickInput.Font = Enum.Font.Gotham
BringNickInput.TextSize = 12
BringNickInput.ClearTextOnFocus = false
BringNickInput.Parent = BringContent

local BringNickCorner = Instance.new("UICorner")
BringNickCorner.CornerRadius = UDim.new(0, 6)
BringNickCorner.Parent = BringNickInput

local BringToggleBtn = Instance.new("TextButton")
BringToggleBtn.Size = UDim2.new(1, 0, 0, 32)
BringToggleBtn.Position = UDim2.new(0, 0, 0, 48)
BringToggleBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
BringToggleBtn.BorderSizePixel = 0
BringToggleBtn.Text = "Bring Parts: OFF"
BringToggleBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
BringToggleBtn.Font = Enum.Font.GothamBold
BringToggleBtn.TextSize = 12
BringToggleBtn.Parent = BringContent

local BringToggleCorner = Instance.new("UICorner")
BringToggleCorner.CornerRadius = UDim.new(0, 6)
BringToggleCorner.Parent = BringToggleBtn

local BringPartsEnabled = false
local BringPartsConnection

local function StopBringParts()
    if BringPartsConnection then
        BringPartsConnection:Disconnect()
        BringPartsConnection = nil
    end
end

local function StartBringParts()
    StopBringParts()
    BringPartsConnection = RunService.Heartbeat:Connect(function()
        if not BringPartsEnabled or Destroyed then
            StopBringParts()
            return
        end

        local targetNick = BringNickInput.Text
        if targetNick == "" then return end

        local GetPlayerFunc = function(Name)
        ...
        local target = GetPlayerFunc(targetNick)
...
BringToggleBtn.MouseButton1Click:Connect(function()
    BringPartsEnabled = not BringPartsEnabled
    if BringPartsEnabled then
        BringToggleBtn.BackgroundColor3 = Color3.fromRGB(50, 180, 70)
        BringToggleBtn.Text = "Bring Parts: ON"
        StartBringParts()
    else
        BringToggleBtn.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
        BringToggleBtn.Text = "Bring Parts: OFF"
...
-- BRING GUI ABAIXO DO MUSIC PLAYER
CreateButton("Bring GUI", FlingPage, function()
    BringGui.Visible = not BringGui.Visible
end)
...
CreateButton("DESTROY", ConfigPage, function()
	Destroyed = true
	StopFly() StopFlyCar() StopFloat() DisableNoclip() StopAntiFling() StopAntiAFK() StopAntiVoid() StopNoFall() ClearAllESP() StopSpectate() StopLoopTP() StopChatLogs() StopBringParts() ToggleFullbright(false) ToggleFreecam(false) Toggle3DRendering(true)
